...
/**
 * Dedicated flight detail route — own Error Boundary via error.tsx.
 * Reuses the globe focused on one flight so failure isolation stays clear.
 */
export default function TrackFlightPage({
  ...
  const flight = useMemo(
    () => DEMO_FLIGHTS.find((f) => f.id === params.flightId) ?? null,
    [params.flightId],
  );
